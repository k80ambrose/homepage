  ...
  useEffect(() => {
    ...
    const sketch = (p) => {
      ...
      class Orb {
        ...
        move() {
          if (this === draggedOrb) {
            this.pos.x = p.lerp(this.pos.x, p.mouseX + dragOffsetX, 0.3)
            this.pos.y = p.lerp(this.pos.y, p.mouseY + dragOffsetY, 0.3)
            this.vel.set(0, 0)
      ...
      p.mousePressed = () => {
        for (let i = orbs.length - 1; i >= 0; i--) {
          const orb = orbs[i]
          if (p.dist(p.mouseX, p.mouseY, orb.pos.x, orb.pos.y) < orb.baseRadius) {
            draggedOrb = orb
            dragOffsetX = orb.pos.x - p.mouseX
            dragOffsetY = orb.pos.y - p.mouseY
            break
          }
        }
      }
